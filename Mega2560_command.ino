...
#define DHT1PIN 8                        // PIN №8 подключения датчика DTH11 в прихожей
...
int action = 1;    //  0/1  - запрос выдачи данных / команда на исполнение
...
void loop() {
  ...
  // полyчение данных от локальных датчиков
  int humDhtInside = dht1.readHumidity();
  int tempDhtInside = dht1.readTemperature();
  int humDhtOutside = dht2.readHumidity();
  int tempDhtOutside = dht2.readTemperature();
